...
app.get("/ping", (request, response) => {
  ...
  for (let i = 0; i < addlist.length; i++) {
    ...
    if (MinutesAgo(o.joined) > 30) {
      ...
      client.users
        .get(o.id)
        .send("```Your Pending Authorization has Expired.```");
    }
  }
  addlist = [];
  addlist = userdb
    .get("users")
    .filter({ role: "homeless" })
    .value();
  if (addlist.length > 0) {
    console.log("Clearing homeless users");
  }
  for (let i = 0; i < addlist.length; i++) {
    let o = addlist[i];
    if (MinutesAgo(o.expiration) > 1440) {
      ...
      try {
        client.users
          .get(o.id)
          .send(
            "```Your account was deleted for being 24 hours without a home server.```"
          );
      } catch (err) {}
    }
  }
...
translate.key =
  "trnsl.1.1.20181221T193920Z.74b4574f7877de0d.b1662dd59cd68fa428a6d312ae3fe51efa23f893";
const languages = [
  "az",
  "sq",
  "am",
...
language = "en"
    testText = await translate(text, { from: user.language, to: language });
    //console.log(language,"->",testText)
    testText = text + "\n**__" + user.language + "__**->**__"+language+"__** " + testText;
  } else {
    testText = false;
  }
...
app.post("/authorization", function(req, res) {
  ...
  let userdata = userdb
    .get("users")
    .find({ internalHash: dat.token })
    .value();
  if (!userdata) {
    return res.send(JSON.stringify({ status: 402, dat: dat }));
  }
...
app.get("/readterms", function(req, res) {
  res.sendFile(path.join(__dirname + "/IO/ViewTerms.html"));
});
var url = require("url");
...
client.on("guildDelete", guild => {
  try {
    ...
    if (affectedusermap === []) {
      globalAnnounce(
        makeEmbed(
          "The link to '" + guild.name + "' was severed..",
          "Affected users: 0"
        )
      );
      return;
    ...
    for (let i = 0; i < affectedusermap.length; i++) {
      ...
      try {
        client.users
          .get(affectedusermap[i].id)
          .send(
            "Your home doorway was deleted. To set a new home use g.sethome on a network channel. If you no longer have access our home channel is https://discord.gg/6y2A4Pk\nTo invite me to your world use the following link: https://isekai.glitch.com"
          );
        userlist +=
          "\n" +
          affectedusermap[i].nick +
          " (<@" +
          affectedusermap[i].id +
          ">)";
      } catch (err) {}
    }
    let desc = "Affected users: " + affectedusermap.length + userlist;
    globalAnnounce(
      makeEmbed("The link to '" + guild.name + "' was severed..", desc)
    );
    return;
  } catch (err) {}
});
client.on("channelDelete", c => {
  try {
    ...
    if (affectedusermap === []) {
      globalAnnounce(
        makeEmbed(
          "A Doorway in " + c.guild.name + " has Closed.",
          "Affected souls: 0"
        )
      );
      return;
    ...
    for (let i = 0; i < affectedusermap.length; i++) {
      ...
      client.users
        .get(affectedusermap[i].id)
        .send(
          "Your home doorway was deleted. To set a new home use g.sethome on a network channel. If you no longer have access our home channel is https://discord.gg/QEyr7bx\nTo invite me to your world use the following link: https://discordbots.org/bot/497371043292381199"
        );
    }
    let desc = "Affected users: " + affectedusermap.length + userlist;
    globalAnnounce(
      makeEmbed("A Doorway in " + c.guild.name + " has Closed.", desc)
    );
    return;
  } catch (err) {}
});
    ...
    if (link.nick !== "!undefined") {
      globalAnnounce(
        makeEmbed(
          ">>" +
            link.nick +
            " has Left their home server. Nickname given a 24 hour grace period."
        )
      );
    }
...
client.on("guildCreate", guildCreate => {
  ...
  guildCreate.createChannel("Isekai", "text").then(channel => {
    channel.send("Connecting to Network...");
    let message = {};
  ...
  globalAnnounce(
    makeEmbed(
      "A connection to the world known as '" +
        guildCreate.name +
        "' was found. (Users: " +
        guildCreate.memberCount +
        ")"
    )
  );
});
...
client.on("message", message => {
  ...
  try {
    ...
    if (
      message.content.toLowerCase().startsWith("g.eval") &&
      message.author.id === "174609192340946944"
    ) {
      try {
    ...
    if (
      ...
      db
        .get("links")
        .value()
        .indexOf(message.channel.id) !== -1
    ) {
      message
        .reply("You are banned from Isekai (" + isBanned.reason + ")")
        .then(c => {
          message.delete();
          c.delete(4000);
        });
    ...
    if (
      message.content === "g.init" &&
      message.channel.permissionsFor(message.member).has("ADMINISTRATOR", true)
    ) {
      init(message, false);
      message.delete();
      return;
    ...
    if (message.webhookID !== null) {
      //ignore bot relay messages
      return;
    }
    if (
      message.author.bot &&
      message.author.tag !== "D&D 5e Campaign Manager#2108"
    ) {
      //ignore bot messages (comment out return to toggle)
      return;
    }
    ...
    if (links.indexOf(message.channel.id) !== -1) {
      /// Was the message sent in an Isekai channel
      //console.log(message)
      let username = message.author.username;
      if (
        username.replace(/[^\x00-\x7F]/g, "") === "" ||
        username.startsWith("[")
      ) {
      ...
      console.log(
        intToRGB(hashCode(message.author.id)),
        usernameOverride + ": " + message.content
      );
      //let lst = arrayRemove(links, message.channel.id)
      let lst = links;
      if (usernameOverride === "!undefined" && role === "unregistered") {
        message
          .reply("Before you may talk on our network you must activate your account. Instructions have been sent to your DMs"
          )
          .then(m => {
            client.users
              ...
              .send(
                makeEmbed(
                  "By linking your discord account you agree to our terms of use.",
                  "https://isekai.glitch.me/readterms\nFollow All of our rules.\nListen to Staff\nNo Profanity\nTo agree to our terms type `I Agree To The Terms`"
                )
              );
            m.delete(18000);
          });
        ...
        message
          .reply("Set your username with g.nick <nickname>")
          .then(m => {
            m.delete(8000);
          });
        message.delete();
      ...
      userdb
        ...
        .assign({ messages: user.messages + 1 })
        .write();
      message.delete();
    }
  } catch (err) {
...
function processDM(message) {
  ...
  if (message.content.startsWith("g.")){
    return processCommand(message,user)}
  if (message.content.toLowerCase()=="i agree to the terms"){
    userdb
    .get("users")
    .find({
      id: message.author.id
    ...
    .write();
    message.reply("Account Registered. Use `g.nick` in your primary server's Isekai channel to set your name.")
  }
}
/* globalAnnounce
  ...
  try {
    ...
    theChannel.fetchWebhooks().then(hook => {
            ...
            var embed = {
              embed: {
                ...
                title:
                  ">>" +
                  tChannel.guild.name +
                  " has connected a channel to the network."
              }
            };
      ...
      if (message.author) {
        ...
        //RankConfig
        avatarURL =
          "https://cdn.glitch.com/0a7482ac-9b81-429d-b27a-5a8b05fc0668%2Fdiscordbackground.png?1548617880988"; //Default
      ...
      */
        ...
        if (message.author.bot || roles.indexOf("BOT") != -1) {
          ...
          avatarURL =
            "https://cdn.glitch.com/0a7482ac-9b81-429d-b27a-5a8b05fc0668%2Fbot.png?1548619665465";
        } else if (message.author.id === "174609192340946944") {
          avatarURL =
            "https://cdn.glitch.com/0a7482ac-9b81-429d-b27a-5a8b05fc0668%2Fowner.png?1548618196207";
          username = "[Owner] " + username;
        } else if (roles.indexOf("ADMIN") != -1) {
          avatarURL =
            "https://cdn.glitch.com/0a7482ac-9b81-429d-b27a-5a8b05fc0668%2FAdmin.png?1548618970869";
          username = "[Admin] " + username;
        } else if (roles.indexOf("MODERATOR") != -1) {
          avatarURL =
            "https://cdn.glitch.com/0a7482ac-9b81-429d-b27a-5a8b05fc0668%2FModerator.png?1548618967493";
          username = "[Mod] " + username;
        } else if (roles.indexOf("EGG") != -1) {
          avatarURL =
            "https://cdn.glitch.com/0a7482ac-9b81-429d-b27a-5a8b05fc0668%2FEGGPFP.png?1548619266497";
      ...
      MSG = input.replace(re, x => {
        if (message.channel.id === channelID) {
          message
            .reply(
              "Please do not ping. Refer to g.help for how this bot handles pinging. Thanks"
            )
            .then(c => {
              c.delete(8000);
            });
        ...
        if (user) {
          ...
          if (blacklist !== undefined) {
            if (blacklist.blockedUsers.indexOf(user.id) !== -1) {
              userstring =
                "**@BlacklistedUser " + intToRGB(hashCode(user.nick)) + "**";
            }
          }
        }
    ...
    */
      ...
      if (blacklist !== undefined) {
        if (blacklist.blockedUsers.indexOf(fromuserid) !== -1) {
          ...
          content.avatarURL =
            "https://cdn.glitch.com/0a7482ac-9b81-429d-b27a-5a8b05fc0668%2FScreenshot_6.png?1542217371514";
          if (content.files) {
...
function processCommand(message, user) {
  try {
    ...
    if (
      message.content.startsWith("g.embed") &&
      user.rank.indexOf("MODERATOR") != -1
    ) {
    ...
    //global Announcement
    if (
      message.content.startsWith("g.ann]") &&
      user.rank.indexOf("ADMIN") != -1
    ) {
    ...
    //Ban User
    if (
      message.content.startsWith("g.ban") &&
      user.rank.indexOf("ADMIN") != -1
    ) {
    ...
    if (
      message.content.startsWith("g.unban") &&
      user.rank.indexOf("ADMIN") != -1
    ) {
    ...
    if (
      message.content.startsWith("g.mute") &&
      (user.rank.indexOf("ADMIN") != -1 || user.rank.indexOf("MODERATOR") != -1)
    ) {
      let t = message.content.slice(7);
      t = t.split(";");
      let tuser = t[0];
      let treason;
      if (!t[1]) {
    ...
    if (
      message.content.startsWith("g.unmute") &&
      (user.rank.indexOf("ADMIN") != -1 || user.rank.indexOf("MODERATOR") != -1)
    ) {
    ...
    if (message.content.startsWith("g.nick ") && user.muted == false) {
      userdb.read()
      //console.log("Nick Running, "+user)
      let nick = filter.clean(message.content.slice(7));
      if (nick.length < 2 || nick.length > 24) {
        message
          .reply("Nicknames Must be between 2 and 24 characters")
          .then(m => m.delete(8000));
        return;
      }
      let doesExist = userdb
        .get("users")
        .find({
          nick: nick
        })
      ...
      if (
        ...
        message
          .reply(
            "Name Not Allowed, Invalid Characters: (`!`, `[`, `]`, `<`, `>`, `\\`, `'`, `\"`, `(`, `)`)"
          )
          ...
          .then(webhook => {
            ...
            var embed = {
              embed: {
                color: 534636,
                title:
                  "A new doorway has opened in '" + tChannel.guild.name + "'"
              }
            };
            if (!skipDoorway) {
              globalAnnounce(embed);
...
function sendHelp(message) {
  ...
  for (let i = 0; i < channels.length; i++) {
    helpmsg +=
      "__" +
      String(client.channels.get(channels[i]).guild.name) +
      "__\n<#" +
      channels[i] +
      ">\n";
  }
...
function mute(nick, reason) {
  ...
  userdb
    ...
    .write();
  globalAnnounce(makeEmbed(">>" + nick + " was Muted. (" + reason + ")"));
} //
    ...
    client.guilds
      ...
      .then(member => {
        response =
          ...
          "\nMain Server: " +
          client.guilds.get(user.primaryServer).name;
        let avatarURL =
